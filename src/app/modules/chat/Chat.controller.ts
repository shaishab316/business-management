...
export const ChatControllers = {
  create: catchAsync(async ({ user, params }, res) => {
    const data = await ChatServices.create([user.id, params.userId]);
  ...
  list: catchAsync(async ({ query, user }, res) => {
    const { chats, meta } = await ChatServices.list(query, user.id);
  ...
  delete: catchAsync(async ({ user, params }, res) => {
    const message = await ChatServices.delete(params.chatId, user.id);
